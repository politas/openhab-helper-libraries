...
class CoreUtilsTestCase(unittest.TestCase):
    def get_item_state_returns_equality(self, test_item, test_type, test_value, expected, 
                                    return_type=None, unit=None):
    ...
    def test_get_item_state_returns_none_for_uninitialized_item_with_no_default(self):
        item = add_item("DogeUninitialized", item_type="String")
        time.sleep(0.5)
        result = get_item_state("DogeUninitialized")
        remove_item(item)
        self.assertIsNone(result, 
            msg="Function get_item_state on uninitialized item with no default should be None but returned '{}'".format(result))
    ...
    def test_get_item_state_returns_datetime_for_date_item_with_return_type(self):
        test_value = DateTimeType("1991-12-21T12:21:19")
        self.get_item_state_returns_correct_type("DogeDate", "DateTime", test_value, datetime,
                                                return_type=datetime)

    def test_get_item_state_returns_ZonedDateTime_for_date_item_with_return_type(self):
        test_value = DateTimeType("1991-12-21T12:21:19")
        self.get_item_state_returns_correct_type("DogeDate", "DateTime", test_value, ZonedDateTime,
                                                return_type=ZonedDateTime)
